...
00000000004017a8 <getbuf>:
  # Using gdb, we find that %rsp, before the subtraction below, is 0x5561DCA0
  # so 0x5561DCA0 points to return address after getbuf returns.
  4017a8:	48 83 ec 28          	sub    $0x28,%rsp         # allocate 0x28 (40) bytes on stack
  4017ac:	48 89 e7             	mov    %rsp,%rdi          # rdi = rsp = address of buffer
...
# Phase 1: When getbuf returns, make it go to touch1.
# - Examining the disassembled code of getbuf, we see that it allocates 0x28 (40) bytes on the stack.
# - This means that we can input a string of 40 bytes of random stuff followed by the address of touch1.
# - Note that because the machine is little-endian, the address of touch1, 0x4017C0, needs to be reversed.
...
# Phase 2: When getbuf returns, pass control to touch2 and pass cookie as its argument.
# - Recall that the first argument of function is in %rdi.
# - We need to alter the return address to the address of touch2, and also set %rdi to cookie.
